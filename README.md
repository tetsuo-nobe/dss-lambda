...
1. **コード** タブのコードで、24行目と 25行目の `blue` を `green` に変更します。
...
1. **コードソース** のコードの 8行目と 14行目の先頭の # を削除します。

1. ページ左側にある **Deploy**　を選択します。

1. **テスト** タブを選択し、**テスト** を選択します。
    - 関数のコードを変更した後の初回実行なので、コールドスタートとなります。
    - 初回のテスト実行後、**実行中の関数: 成功** の **詳細**　を展開表示し、**初期所要時間** が表示されていることを確認します。
    - **ログ出力** の中の **outside_handler** と **inside_handler** に表示されている日時を確認します。
     
1. 再度　**テスト** を選択します。
    - **実行中の関数: 成功** の **詳細**　を展開表示し、**初期所要時間** が表示されていないことを確します。
    - **ログ出力** の中の **outside_handler** と **inside_handler** に表示されている日時を確認します。
    - **outside_handler** の値は変更されていないことを確認します。
        - これはハンドラ関数の外側のコードで　**outside_handler** の値を取得しており、コールドスタート時に取得した値を再利用しているためです。
...
1. **コード** タブを選択して、24行目と 25行目の `green` を `red` に変更します。
